...
class Gaussian_Scene():  
    ...
    def _render_RGBD(self,frame,background_color='black'):
        ...
        # render
        render_out,render_alpha,meta = gs.rendering.rasterization(means = xyz,
                                                scales    = scale,
                                                quats     = rotation,
                                                opacities = opacity,
                                                colors    = rgb,
        ...
        return render_rgb, render_dpt, render_alpha, gaussian_ids

    def _return_xyz(self):

        xyz       = torch.cat([gf.xyz.reshape(-1,3) for gf in self.gaussian_frames],dim=0)
        return xyz

    def _return_rgb(self):
        rgb       = torch.cat([gf.rgb.reshape(-1,3) for gf in self.gaussian_frames],dim=0)
        # activate
        rgb       = self.rgbs_act(rgb)
        return rgb
    ...
    def _render_for_inpaint(self,frame):
        # first render
        render_rgb, render_dpt, render_alpha, _ = self._render_RGBD(frame)

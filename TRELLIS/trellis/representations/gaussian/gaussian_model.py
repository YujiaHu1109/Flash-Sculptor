...
class Gaussian:
    ...
    def get_gaussians(self):
        gaussian = {
            'xyz': self._xyz,  # 点云坐标
            'features_dc': self._features_dc,  # 球谐 DC 特征
            'features_rest': self._features_rest if self._features_rest is not None else None,
            'opacity': self._opacity,  # 不透明度
            'scaling': self._scaling,  # 缩放属性
            'rotation': self._rotation,  # 旋转属性
        }
        outputs = {'gaussian': [gaussian]}
        return outputs
    
    def rotate_around_x_axis(self, angle_degrees):
        ...
        self.from_xyz(rotated_xyz)

    def rotate_around_y_axis(self, angle_degrees):
        angle_radians = math.radians(angle_degrees)
        rotation_matrix = torch.tensor([
